...
/// Antigravity 自定义 Skill 安装弹窗
class _AntigravityCustomSkillInstallDialog extends StatefulWidget {
  final VoidCallback onInstalled;

  const _AntigravityCustomSkillInstallDialog({required this.onInstalled});

  @override
  State<_AntigravityCustomSkillInstallDialog> createState() =>
      _AntigravityCustomSkillInstallDialogState();
}

class _AntigravityCustomSkillInstallDialogState
    extends State<_AntigravityCustomSkillInstallDialog> {
  ...
  void _showInstallFormDialog() {
    showDialog<void>(
      context: context,
      builder: (context) => _AntigravitySkillInstallFormDialog(
  ...
  Future<void> _doInstall(String url, String skillName) async {
    ...
    try {
      ...
      // Antigravity skills 目录: ~/.gemini/antigravity/skills/
      final home = PlatformUtils.userHome;
      final skillDir = Directory(
          PlatformUtils.joinPath(home, '.gemini', 'antigravity', 'skills', skillName));
      ...
      // Antigravity 使用 SKILL.md 作为技能定义文件
      ...
      if (mounted) {
        Toast.show(
            context, message: '${S.get('skill_install_failed')}: $e', type: ToastType.error);
      }
    }
  }
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    S.get('antigravity_custom_skill_desc'),
                        ...
                        child: Row(
                          ...
                          children: [
                            if (_isInstalling)
                              ...
                              const Icon(Icons.add_link, size: 18, color: Colors.purple),
                            const SizedBox(width: 8),
                            Text(
                              S.get('custom_skill_install'),
  ...
  Widget _buildCommunityResourcesSection(bool isDark) {
    // Antigravity 社区资源
    const awesomeRepos = [
      {'name': 'antigravity-skills', 'url': 'https://github.com/topics/antigravity-skills'},
    ];
    const skillDirectories = [
      {'name': 'antigravity.google', 'url': 'https://antigravity.google/docs/skills'},
    ];
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            const Icon(Icons.explore, size: 18, color: Colors.purple),
            const SizedBox(width: 8),
            Text(
...
/// Antigravity Skill 安装表单弹窗
class _AntigravitySkillInstallFormDialog extends StatefulWidget {
  final Future<void> Function(String url, String name) onInstall;

  const _AntigravitySkillInstallFormDialog({required this.onInstall});

  @override
  State<_AntigravitySkillInstallFormDialog> createState() => _AntigravitySkillInstallFormDialogState();
}

class _AntigravitySkillInstallFormDialogState extends State<_AntigravitySkillInstallFormDialog> {
