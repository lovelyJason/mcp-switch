...
/// Gemini 自定义 Skill 安装弹窗
class _GeminiCustomSkillInstallDialog extends StatefulWidget {
  final VoidCallback onInstalled;

  const _GeminiCustomSkillInstallDialog({required this.onInstalled});

  @override
  State<_GeminiCustomSkillInstallDialog> createState() =>
      _GeminiCustomSkillInstallDialogState();
}

class _GeminiCustomSkillInstallDialogState
    extends State<_GeminiCustomSkillInstallDialog> {
  ...
  void _showInstallFormDialog() {
    showDialog<void>(
      context: context,
      builder: (context) => _GeminiSkillInstallFormDialog(
  ...
  Future<void> _doInstall(String url, String skillName) async {
    ...
    try {
      ...
      // Gemini skills 目录: ~/.gemini/skills/
      final home = PlatformUtils.userHome;
      final skillDir = Directory(
          PlatformUtils.joinPath(home, '.gemini', 'skills', skillName));
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    S.get('gemini_custom_skill_desc'),
  ...
  Widget _buildCommunityResourcesSection(bool isDark) {
    // Gemini 社区资源
    const awesomeRepos = [
      {'name': 'gemini-cli-extensions', 'url': 'https://github.com/gemini-cli-extensions'},
    ];
    const skillDirectories = [
      {'name': 'geminicli.com', 'url': 'https://geminicli.com/'},
    ];
    ...
    return Column(
      ...
      children: [
        ...
        Row(
          children: [
            const Icon(Icons.explore, size: 18, color: Colors.green),
            const SizedBox(width: 8),
            Text(
...
/// Gemini Skill 安装表单弹窗
class _GeminiSkillInstallFormDialog extends StatefulWidget {
  final Future<void> Function(String url, String name) onInstall;

  const _GeminiSkillInstallFormDialog({required this.onInstall});

  @override
  State<_GeminiSkillInstallFormDialog> createState() => _GeminiSkillInstallFormDialogState();
}

class _GeminiSkillInstallFormDialogState extends State<_GeminiSkillInstallFormDialog> {
